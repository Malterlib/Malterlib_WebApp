...
namespace NMib::NWebApp::NWebAppManager
{
	...
	TCFuture<void> CWebAppManagerActor::fp_SetupPrerequisites_NodeExtract()
	{
		...
		g_Dispatch(*mp_FileActors)
			...
			() mutable -> TCFuture<CNodeInfo>
			{
				...
				if (bDoInstall)
				{
					ThisActor(&CWebAppManagerActor::f_ExtractTar, DistFile, ProgramDirectory) > Promise / [=]
						{
							try
							{
	...
	TCFuture<void> CWebAppManagerActor::fp_SetupPrerequisites_Package(CStr const &_PackageName, CWebAppManagerOptions::EPackageType _Type)
	{
		TCPromise<void> Promise;
		...
		;
		auto DefaultUser = (_Type == CWebAppManagerOptions::EPackageType_FastCGI) ? mp_FastCGIUser : (_Type == CWebAppManagerOptions::EPackageType_Websocket) ? mp_WebsocketUser : mp_NodeUser;
			...
			() mutable -> TCFuture<CPackageInfo>
			{
				...
				try
				{
					...
					if (bDoInstall)
					{
						...
						ThisActor(&CWebAppManagerActor::f_ExtractTar, PackageFileName, ProgramDirectory) > InstallPromise / [=]() mutable
							{
